...
A return to ADR-0005's stance: the agent is the loop driver. v2.x grew a per-feature scheduler, a planning state, a persona orchestration layer, a coverage gate, a handoff store, a reminders bridge, and ~12 transition reasons. v3 keeps the lifecycle FSM, the evidence ledger with its completion gates, and the Ralph reprompt — and removes everything else.
...
- `pause({ reason? })`
- `resume()`
- `complete()` — gated: every VAL pass + fresh evidence + because-on-manual + REPORT.md gate
- `abandon({ reason })` — reason required

### `charter_record`

Writes. Actions:
- `evidence({ entries: [{ criterionId, outcome, summary, because?, source, recordedBy?, details?, artifacts? }] })` — manual or batch; atomic per call. Per ADR-0013 this is the only action; the agent runs any checks itself and records the result here (charter never spawns commands).
...
- `requireReviewSubagent` — per-VAL display-only annotation. `source` / `recordedBy` provenance is surfaced for confidence, not used as a gate. Zero bundled personas; user invokes their own.
...
- Bundled personas (all persona directories removed; users bring their own subagents)
...
- Structured logging from m2-logging (lifecycle transitions, evidence recording, completion blockers, ralph trace at debug)
...
5. **m3-personas-out.** Delete `agents/*` directory. Remove every reference to specific persona names from src/ and SKILL.md. `requireReviewSubagent` is display-only; provenance is shown but not gated.
