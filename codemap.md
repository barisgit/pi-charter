...
Pi extension for durable charter-bound agent work. Provides: objective/criteria/evidence lifecycle, macro feature plans with milestone gates, smart-Ralph steering via post-turn evaluator, and a terminal widget. Successor to pi-goals v1.

## System Entry Points

| File | Role |
|---|---|
| `src/index.ts` | Extension registration entry point. Wires all tools, commands, flags, and evaluator hook to the pi-coding-agent host. |
...
|---|---|---|
| `src/application/` | Tool surface & orchestration: charter lifecycle state machine, evidence recording, plan CRUD, drift views, evaluator, hooks, async bridge, reminders bridge. | [View Map](src/application/codemap.md) |
...
4. **Post-turn LLM evaluator** — `evaluator-service.ts` runs after every agent turn; injects steer hints via `reason` field, skips redundant re-evaluations.
5. **Veto hook bus** — `hooks.ts` exposes `before_lock_plan`, `before_complete`, `before_amend_charter`, `before_force_complete` for in-process extension customization.
6. **Milestone `ready_for_review` projection** — `record-service.ts` synthesizes this tri-state field from per-criterion `requireReviewSubagent` flags; propagates through feature state → charter state.
7. **Pi-subagents async bridge** — `async-bridge-service.ts` subscribes to `pi.events` for charter subagent completions and appends attributed events to `events.jsonl`.
8. **Session↔charter dual binding** — `binding-service.ts` maintains bidirectional pointers; reconciles after unexpected process restarts.
9. **Pure reducer → ViewModel** — UI layer is stateless string rendering: `loadCharterSnapshot` → reducer → `buildViewModel` → renderer.
...
pi-coding-agent host
  └─ src/index.ts (registration)
       ...
       │    ├─ application/evaluator-service.ts (post-turn LLM reasoner)
...
| `docs/implementation/` | Architecture, lifecycle, tool contracts, evaluator specs. |
