...
- `charter-picker.ts` exports `createCharterPickerOverlay(opts)`, a fullscreen-component factory that builds the master-detail picker on the `pi-extension-utils` `paneOverlay()` helper. The options are constructed inside the factory so the content callbacks close over `theme` (paneOverlay passes only a `PaneOverlayContext`, not the theme) and over mutable closure state (objective/all expansion toggles, short-lived flash messages). It renders a left charter list plus info and auto-derived legend zones and a right detail pane with objective, completion blockers or readiness, milestone/VAL tree, and recent evidence. It is read-only: it only ever closes with `null`.
- `charter-picker-constants.ts` centralizes picker terminal status, layout bounds, split step, banned keys, flash TTL, and left-row column widths.
...
8. `paneOverlay` handles navigation (`j/k`, arrows, `u/d` half-page, `g/G`), focus (`tab`/`←`/`→`), and split resize (`[`/`]`); the picker adds custom actions for detail-pane expansion (`space` fold, `o` objective — both gated to detail focus), open charter dir (`shift+o`/`O`), and copy charter id (`y`), a `s` sidebar collapse, and close (`q`/`esc`/`ctrl+c`). `enter`, delete, and printable `b/r/p/a/c` are intentionally banned/no-op.

## Integration

- `pi-extension-utils`: `charter-picker.ts` builds on `paneOverlay()` (plus the `clipText`/`clipStyled`/`padRight` chrome helpers and the `FullscreenComponentFactory`/`PaneOverlay*` types); the returned component implements the `{ render, handleInput, dispose }` shape.
- `@earendil-works/pi-tui`: `widget.ts` uses `truncateToWidth`; picker uses `visibleWidth`.
