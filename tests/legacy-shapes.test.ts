...
async function seedPlanningCharter(input: { projectDir: string; homeDir: string; sessionId: string; charterId: string }) {
  ...
  await writeFile(
    ...
    [
      ...
      "Description: legacy shape evidence is accepted.",
      "Verifier: manual",
      "Because: manual probe for legacy",
      "### VAL-L-002 second criterion",
      "Description: covered by the batch no-warning fixture.",
      "Verifier: manual",
      "Because: manual probe for legacy",
      ...
      await tools.get("charter_plan")!.execute(
        "call-1",
        {
          ...
          features: [
            ...
            { id: "f-batch-b", milestone: "m1", order: 10, fulfills: ["VAL-L-002"], body: `batch b\n\n${VALIDATION_MD}` },
          ],
        },
        new AbortController().signal,
        () => {},
