...
async function makeActiveCharter(projectDir: string, charterId: string): Promise<string> {
  ...
  await writeFile(
    ...
    [
      ...
      "## Criteria",
      "",
      ...ids.map((id, index) => [`### ${id} — Criterion ${index}`, `Description: Criterion ${index}.`, "Verifier: manual", "Because: test fixture rationale", ""].join("\n")),
    ].join("\n"),
    "utf8",
  );
    ...
    await withTempProject(async (projectDir) => {
      ...
      await Promise.all(Array.from({ length: 6 }, (_, index) => {
        ...
        return applyHandoff(projectDir, {
          charterId,
          featureId: "f1",
          subagentSessionId: `charter-reviewer-${index}`,
