...
async function withTempProject<T>(fn: (projectDir: string) => Promise<T>): Promise<T> {
  const projectDir = await mkdtemp(join(tmpdir(), "pi-charter-commands-"));
  try {
...
function charterMd(commandsSection = ""): string {
  return [
    ...
    "Verifier: command",
    "Command: bun test tests/charter-commands-section.test.ts",
...
describe("charter Commands section", () => {
  ...
  test("duplicate keys: last-write-wins with warning", () => {
    const parsed = parseCharterMarkdown(charterMd("## Commands\n\ntest: bun test\ntest: bun test tests/charter-commands-section.test.ts"));
    expect(parsed.commands).toEqual({ test: "bun test tests/charter-commands-section.test.ts" });
    expect(parsed.warnings).toEqual([
