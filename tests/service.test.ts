...
describe("charter service", () => {
  test("create returns planning state with legal next actions", async () => {
    await withTempProject(async (projectDir) => {
      ...
      expect(result.nextActions.map((a) => `${a.tool}:${a.action}`)).toContain("charter_plan:view");
      expect(result.nextActions.map((a) => `${a.tool}:${a.action}`)).toContain("charter_plan:lock_plan");
  ...
  test("pause and resume preserve previous status", async () => {
    await withTempProject(async (projectDir) => {
      ...
      expect(paused.nextActions.map((a) => `${a.tool}:${a.action}`)).toContain("charter_manage:resume");
